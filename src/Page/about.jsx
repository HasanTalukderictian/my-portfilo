...
const experiences = [
    { company: 'Akashbari Holidays', role: 'Software Engineer', period: '2026 – Present' },
    { company: 'Paperfly Private', role: 'Software Engineer', period: '2024 – 2025' },
    { company: 'F3 Solution Limited', role: 'Software Engineer', period: '2023 – 2024' },
    { company: 'Wetech Digital', role: 'Software Engineer', period: '2023 – 2024' },
];
    ...
    return (
        <section id={id} className="bg-slate-50 px-5 py-20">
            <div className="mx-auto max-w-6xl">
                <SectionTitle title="About me" subtitle="A little about who I am and where I've worked." />

                <div className="grid gap-14 md:grid-cols-2">
                    {/* Bio + stats */}
                    <div>
                        <p className="text-lg leading-relaxed text-slate-700">
                            I'm <span className="font-semibold text-slate-900">Hasan Talukder</span>, a full-stack web
                            developer, software designer and mobile developer. I've spent the last few years building
                            web apps and I have a solid grasp of modern UI design principles.
                        </p>

                        <dl className="mt-10 grid grid-cols-2 gap-x-6 gap-y-8">
                            {stats.map((s) => (
                                <div key={s.label} className="border-l-2 border-teal-700 pl-4">
                                    <dt className="sr-only">{s.label}</dt>
                                    <dd className="font-display text-4xl font-bold text-slate-900">
                                        <CountUp start={0} end={s.value} duration={2.5} suffix={s.suffix} />
                                    </dd>
                    ...
                    {/* Experience timeline */}
                    <div>
                        <h3 className="font-display mb-6 text-xl font-bold text-slate-900">Work experience</h3>
                        <ol className="relative space-y-8 border-l border-slate-300 pl-8">
                            {experiences.map((exp, i) => (
                                <li key={i} className="relative">
                                    <span className="absolute -left-[38px] top-1.5 h-3 w-3 rounded-full border-2 border-teal-700 bg-slate-50"></span>
                                    <p className="text-sm font-medium text-teal-700">{exp.period}</p>
                                    <p className="font-display text-lg font-semibold text-slate-900">{exp.company}</p>
